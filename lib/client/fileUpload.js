...
Template.afFileUpload.onCreated(function () {
  ...
  instance.collection = Mongo.Collection.get ?
      Mongo.Collection.get(instance.data.atts.collection).filesCollection
      : (global[this.data.atts.collection] ||
      Meteor.connection._mongo_livedata_collections
      [instance.data.atts.collection].filesCollection);
  ...
  if (!isNaN(instance.insertConfig.chunkSize) ||
  ...
  if (!instance.collection) {
    ...
    // 404?
    throw new Meteor.Error(404, '[meteor-autoform-files] No such collection "'+
  ...
  instance.formId        = instance.data.atts.id;
